Ext.define('Testtwo.view.reportui.datachart.chart.ChartController', {
	...
	initObject : function() {
		...
		//var clayout=this.reportJSON.displayType!=1?this.getView().down("#chartcolumnlayout"):this.getView().tools[0];
		if(clayout!=null && clayout!=undefined){
			if(this.reportJSON != undefined && this.reportJSON.defaultCColumn!=undefined && this.reportJSON.defaultCColumn.toString().length>0){
				clayout.value=parseInt(this.reportJSON.defaultCColumn);
				clayout.minLength=parseInt(this.reportJSON.defaultCColumn);
			}
		}
	},
	afterRender:function()
	{
		if(this.reportView.isCustomReport==false && this.reportJSON.displayType==0)
			this.loadCharts(this.chartJson);
	...
	onPanelResize:function(me, width, height, oldWidth, oldHeight, eOpts )
	{
		...
		if(width<768){
			newColumns=1;
			me.down("ratingField").setValue(1);
		}else{
			//newColumns=parseInt(me.down("ratingField").value);
			newColumns=this.reportJSON.defaultCColumn==null?1:parseInt(this.reportJSON.defaultCColumn);
			me.down("ratingField").setValue(newColumns);
		}
		//newColumns=parseInt(me.down("ratingField").value);
		...
		for ( var k in FusionCharts.items) {
				...
				if(fusionObj.args.hasOwnProperty("group") && fusionObj.args.group == true){
					...
					var panelbodyId = fusionObj.options.containerElementId;
					if (me.down('#'+ panelbodyId.substring(0, panelbodyId
											.indexOf('-body'))) != null) {
