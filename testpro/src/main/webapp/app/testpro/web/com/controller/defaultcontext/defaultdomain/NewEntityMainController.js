Ext.define('Testpro.testpro.web.com.controller.defaultcontext.defaultdomain.NewEntityMainController', {
     extend: 'Testpro.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.NewEntityMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#NewEntityTree');
     ...
     refreshMainForm: function(but, data, method) {
          ...
          var formPanel = this.view.down('#NewEntity');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#NewEntityGrid');
               var tree = this.view.down('#NewEntityTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#NewEntity');
          var grid = this.view.down('#NewEntityGrid');
          var tree = this.view.down('#NewEntityTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#NewEntityGrid');
               ...
               var formPanel = this.getView().up().down('#NewEntity');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#NewEntityTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#NewEntity').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#NewEntityGrid').store;
          gridStore.removeAll();
     ...
     loadData: function(responseData) {
          this.view.down('#NewEntity').getViewModel().setData(responseData.response.data);
     },
