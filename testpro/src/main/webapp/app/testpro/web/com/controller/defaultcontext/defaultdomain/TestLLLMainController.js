Ext.define('Testpro.testpro.web.com.controller.defaultcontext.defaultdomain.TestLLLMainController', {
     extend: 'Testpro.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.TestLLLMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#TestLLLTree');
     ...
     refreshMainForm: function(but, data, method) {
          ...
          var formPanel = this.view.down('#TestLLL');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#TestLLLGrid');
               var tree = this.view.down('#TestLLLTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#TestLLL');
          var grid = this.view.down('#TestLLLGrid');
          var tree = this.view.down('#TestLLLTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#TestLLLGrid');
               ...
               var formPanel = this.getView().up().down('#TestLLL');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#TestLLLTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#TestLLL').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#TestLLLGrid').store;
          gridStore.removeAll();
     ...
     loadData: function(responseData) {
          this.view.down('#TestLLL').getViewModel().setData(responseData.response.data);
     },
